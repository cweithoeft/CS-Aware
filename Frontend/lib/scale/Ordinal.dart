...
class Ordinal extends Scale {
  ...
  void set domain(List x) {
    ...
    if (_ranger["t"] == "range") {
      ...
      rangeRoundBands(_ranger["a"][0],
          padding: _ranger["a"][1], outerPadding: _ranger["a"][2]);
    }
  }
  ...
  void rangeRoundBands(List x, {num padding: 0, num outerPadding}) {
    ...
    var reverse = x[1] < x[0], start, stop;
    if (reverse) {
      start = x[1];
      stop = x[0];
    } else {
    ...
    var step = ((stop - start) / (_domain.length - padding + 2 * outerPadding))
            .floor(),
